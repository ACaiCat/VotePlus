    ...
    public class VotePlus : TerrariaPlugin
    {
        ...
        private void PlayerHooks_PlayerPostLogin(TShockAPI.Hooks.PlayerPostLoginEventArgs e)
        {
            ...
            if (VoteKicks.ContainsKey(plr.Account.Name))
            {
                ...
                {
                    plr.Disconnect($"你被投票踢出了服务器!解除时间剩余:{(int)(VoteKicks[plr.Account.Name] - DateTime.Now).TotalSeconds}秒");
                }
                else
                {
        ...
        private void VoteHandler(CommandArgs args)
        {
            ...
            {
                args.Player.SendInfoMessage("[i:4344]Vote用法:\n" +
                        "/vote kick <玩家> 投票踢出玩家\n" +
                        "/vote ban <玩家> 投票封禁玩家\n" +
                        "/vote mute <玩家> 投票禁言玩家\n" +
                        "/vote clearboss <BOSS名称> 投票清理BOSS(不掉落)\n" +
                        "/vote clearevent 投票清理地图中所有事件\n" +
                        "/vote day 投票将时间改为白天\n" +
                        "/vote night 将时间改为黑夜\n" +
                        "/vote rain 投票下雨或停止下雨\n" +
                        "/vote topic <主题> 自由主题投票\n" +
                        "/vote clearkick <玩家> 将玩家从踢出状态移除\n" +
                        "/vote kicklist 查看处于踢出状态的玩家");
                return;
            }
            switch (args.Parameters[0].ToLower())
            {
                case "kick":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]投票踢出功能已被禁用!");
                        return;
                    }
                    if (args.Parameters.Count < 2)
                    {
                        args.Player.SendErrorMessage("[i:4344]用法: /vote kick <玩家>");
                        return;
                    }
                    if (Votes != null)
                    {
                        args.Player.SendErrorMessage("[i:4344]当前有正在进行的投票!");
                        return;

                    }
                    if (TShock.UserAccounts.GetUserAccountByName(args.Parameters[1]) == null)
                    {
                        args.Player.SendErrorMessage("[i:4344]目标账号不存在!");
                        return;
                    }
                ...
                case "ban":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]投票封禁功能已被禁用!");
                        return;
                    }
                    if (args.Parameters.Count < 2)
                    {
                        args.Player.SendErrorMessage("[i:4344]用法: /vote ban <玩家>");
                        return;
                    }
                    if (Votes != null)
                    {
                ...
                case "mute":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]投票禁言功能已被禁用!");
                        return;
                    }
                    if (args.Parameters.Count < 2)
                    {
                        args.Player.SendErrorMessage("[i:4344]用法: /vote mute <玩家>");
                        return;
                    }
                    if (Votes != null)
                    {
                ...
                case "clearboss":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]投票清除Boss功能已被禁用!");
                        return;
                    }
                    if (args.Parameters.Count < 2)
                    {
                        args.Player.SendErrorMessage("[i:4344]用法: /vote clearboss <BOSS名称>");
                        return;
                    }
                    if (Votes != null)
                    {
                    ...
                    if (bosses.Count == 0)
                    {
                        args.Player.SendErrorMessage("[i:4344]目标BOSS不存在!");
                        return;
                    }
                    ...
                    if (!bosses[0].boss)
                    {
                        args.Player.SendErrorMessage("[i:4344]目标不是BOSS!");
                    }
                ...
                case "clearevent":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]投票清除事件功能已被禁用!");
                        return;
                    }
                    if (Votes != null)
                    {
                ...
                case "day":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]投票修改时间功能已被禁用!");
                        return;
                    }
                    if (Votes != null)
                    {
                ...
                case "rain":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]投票修改天气功能已被禁用!");
                        return;
                    }
                    if (Votes != null)
                    {
                ...
                case "topic":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]自由主题投票功能已被禁用!");
                        return;
                    }
                    if (args.Parameters.Count < 2)
                    {
                        args.Player.SendErrorMessage("[i:4344]用法: /vote topic <投票主题>");
                        return;
                    }
                    if (Votes != null)
                    {
                ...
                case "kicklist":
                    ...
                    stringBuilder.AppendLine("[i:4344]当前投票踢出列表:");
                    var kickList = VoteKicks.Where(v => v.Value > DateTime.Now);
                    foreach (var kick in kickList)
                    {
                        stringBuilder.AppendLine($"{kick.Key}:剩余{(int)(kick.Value-DateTime.Now).TotalSeconds}秒");
                    }
                ...
                case "clearkick":
                    ...
                    {
                        args.Player.SendErrorMessage("[i:4344]你没有权限这么做!");
                        return;
                    }
                    if (args.Parameters.Count < 2)
                    {
                        args.Player.SendErrorMessage("[i:4344]用法: /vote clearkick <玩家>");
                        return;
                    }
                    if (VoteKicks.ContainsKey(args.Parameters[1]))
                    {
                        VoteKicks.Remove(args.Parameters[1]);
                        args.Player.SendSuccessMessage("[i:4344]成功清除投票踢出!");
                    }
                    else
                    {
                        args.Player.SendErrorMessage("[i:4344]目标玩家不存在投票踢出列表中!");
                    }
                    break;
            ...
            if (Timer == 60)
            {
                ...
                if (Votes != null)
                {
                    ...
                    if (Votes.IsEnd)
                    {
                        ...
                        {
                            TSPlayer.All.SendErrorMessage("[i:4344]投票未通过!\n" +
                                $"*投票人数少于{Config.config.MinVote}人");
                            Votes = null;
                            return;
                        }
                        if (Votes.AgreePercent < Config.config.VotePass)
                        {
                            TSPlayer.All.SendWarningMessage("[i:4344]投票未通过!\n" +
                                $"*赞成:[c/32FF82:{Votes.Argeement.Count()}票],反对:[c/E11919:{Votes.Disargeement.Count()}票],通过率:{Votes.AgreePercent}%({Config.config.VotePass}%)");
                        ...
                        {
                            Votes.VotePass();
                            TSPlayer.All.SendSuccessMessage($"*赞成:[c/32FF82:{Votes.Argeement.Count()}票],反对:[c/E11919:{Votes.Disargeement.Count()}票],通过率:{Votes.AgreePercent}%({Config.config.VotePass}%)");
                            Votes = null;
                            return;
